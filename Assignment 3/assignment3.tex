...
(D) \(\frac{3}{4}\)
\section{Solution}
Let $Y \in \mathbb{N}$ denote the number of times the experiment is performed. $Y=k$ represents $k-1$ failures before getting $1$ success. $p=\frac{1}{2}$ \begin{align}
    \therefore p_{Y}(k) &= (1-p)^{k-1}\times p 
\end{align}  
For getting first success on an odd try, $k = 2n+1$.
Let X be the Bernoulli Random Variable $X \sim B \brak{1,0}$. $X=1$ denotes the outcome that odd number of tries are required to get the first head, $X=0$ denotes all other outcomes. The probability $p_Y(k)$ is mutually exclusive for all odd tries. Hence, by the rule of sum,
\begin{align}
    \pr{X=1} &= \sum_{n=0}^{\infty}p_Y(2n+1)\\
    \pr{X=1} &= \sum_{n=0}^{\infty}(1-p)^{2n}\times p\\
    \pr{X=1} &= \frac{p}{1 - (p^2 - 2p + 1)}\\
    \pr{X=1} &= \frac{1}{2-p}\\
